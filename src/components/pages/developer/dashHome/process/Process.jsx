import React from "react";
import { devBaseImgUrl } from "../../../../helpers/functions-general";
import { HiPencil } from "react-icons/hi";

const Process = ({
  handleUpdateProcessTitle,
  processData,
  handleUpdateProcessA,
  handleUpdateProcessB,
  handleUpdateProcessC,
  handleUpdateProcessD,
}) => {
  return (
    <>
      <section className="process pt-20 pb-10">
        <div className="customContainer">
          <div className="theTitle leading-1 mb-12 relative">
            <p>
              {processData?.data?.length > 0 &&
              processData.data[0]?.process_subtitle
                ? processData?.data[0].process_subtitle
                : "Subtitle"}
            </p>
            <h2 className="text-[clamp(30px,7vw,45px)] font-semibold text-primary leading-[1.1]">
              {processData?.data?.length > 0 &&
              processData.data[0]?.process_title
                ? processData?.data[0].process_title
                : "Title"}
            </h2>
            <a
              className="absolute cursor-pointer tooltip-btn left-[550px] top-2"
              data-tooltip="Edit contents"
              onClick={handleUpdateProcessTitle}
            >
              <HiPencil className=" bg-[#C7AC27] rounded-full  w-[25px] h-[25px] p-[5px] border-[1px] text-black" />
            </a>
          </div>
          <img
            src={`${devBaseImgUrl}/our-process.png`}
            loading="lazy"
            className="hidden md:block h-full object-contain object-top w-[78%] mx-auto mb-2"
            alt="Our Process"
          />
          <div>
            <div className="grid place-items-center text-justify">
              <div className="md:grid grid-cols-4">
                <div className="processItem md:text-center md:translate-y-[-50px] lg:translate-y-[-100px] mb-6">
                  <div>
                    <img
                      src={`${devBaseImgUrl}/wecollaborate.png`}
                      className="We Collaborate"
                      alt=""
                    />
                  </div>
                  <div>
                    <p className="text-primary font-bold text-lg">
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_title_a
                        ? processData?.data[0].process_title_a
                        : "Title"}
                    </p>
                    <p>
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_description_a
                        ? processData?.data[0].process_description_a
                        : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam deleniti esse et maiores! Minus, praesentium!"}
                    </p>
                    <a
                      className="absolute cursor-pointer tooltip-btn right-0 -top-2"
                      data-tooltip="Edit contents"
                      onClick={handleUpdateProcessA}
                    >
                      <HiPencil className=" bg-[#C7AC27] rounded-full  w-[25px] h-[25px] p-[5px] border-[1px] text-black" />
                    </a>
                  </div>
                </div>
                <div className="processItem md:text-center mb-6 relative">
                  <div>
                    <a
                      className="absolute cursor-pointer tooltip-btn right-0 -top-2"
                      data-tooltip="Edit contents"
                      onClick={handleUpdateProcessB}
                    >
                      <HiPencil className=" bg-[#C7AC27] rounded-full  w-[25px] h-[25px] p-[5px] border-[1px] text-black" />
                    </a>
                    <p className="text-primary font-bold text-lg">
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_title_b
                        ? processData?.data[0].process_title_b
                        : "Title"}
                    </p>
                    <p>
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_description_b
                        ? processData?.data[0].process_description_b
                        : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam deleniti esse et maiores! Minus, praesentium!"}
                    </p>
                  </div>
                </div>
                <div className="processItem md:text-center md:translate-y-[-50px] lg:translate-y-[-100px] mb-6">
                  <div>
                    <a
                      className="absolute cursor-pointer tooltip-btn right-0 -top-2"
                      data-tooltip="Edit contents"
                      onClick={handleUpdateProcessC}
                    >
                      <HiPencil className=" bg-[#C7AC27] rounded-full  w-[25px] h-[25px] p-[5px] border-[1px] text-black" />
                    </a>
                    <p className="text-primary font-bold text-lg">
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_title_c
                        ? processData?.data[0].process_title_c
                        : "Title"}
                    </p>
                    <p>
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_description_c
                        ? processData?.data[0].process_description_c
                        : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam deleniti esse et maiores! Minus, praesentium!"}
                    </p>
                  </div>
                </div>
                <div className="processItem md:text-center relative">
                  <div>
                    <a
                      className="absolute cursor-pointer tooltip-btn right-0 -top-2"
                      data-tooltip="Edit contents"
                      onClick={handleUpdateProcessD}
                    >
                      <HiPencil className=" bg-[#C7AC27] rounded-full  w-[25px] h-[25px] p-[5px] border-[1px] text-black" />
                    </a>
                    <p className="text-primary font-bold text-lg">
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_title_d
                        ? processData?.data[0].process_title_d
                        : "Title"}
                    </p>
                    <p>
                      {processData?.data?.length > 0 &&
                      processData.data[0]?.process_description_d
                        ? processData?.data[0].process_description_d
                        : "Lorem ipsum dolor sit amet consectetur adipisicing elit. Magnam deleniti esse et maiores! Minus, praesentium!"}
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default Process;
