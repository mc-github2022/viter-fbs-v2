...
const ModalUpdatePartnerWithUs = ({ itemEdit, partnerWithUsData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        partnerWithUsData?.data?.length
          ? `${apiVersion}/partnerWithUs/${partnerWithUsData.data[0].partner_with_us_aid}` // update
          : `${apiVersion}/partnerWithUs`, // create
        partnerWithUsData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Partner With Us
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                partner_with_us_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdatePartnerWithUs;
