...
const ModalUpdateBookkeepingBanner = ({ itemEdit, bookkeepingData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        bookkeepingData?.data?.length
          ? `${apiVersion}/bookkeeping/${bookkeepingData.data[0].bookkeeping_banner_aid}` // update
          : `${apiVersion}/bookkeeping`, // create
        bookkeepingData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Bookkeeping Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                bookkeeping_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="bookkeeping_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="bookkeeping_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="bookkeeping_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateBookkeepingBanner;
