...
const ModalUpdateImmersionBanner = ({ itemEdit, immersionData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        immersionData?.data?.length
          ? `${apiVersion}/immersion/${immersionData.data[0].immersion_banner_aid}` // update
          : `${apiVersion}/immersion`, // create
        immersionData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["immersion"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    isUpdateImmersion: itemEdit,
    immersion_banner_title:
      immersionData?.data?.[0]?.immersion_banner_title ?? "",
    immersion_banner_title_bold:
      immersionData?.data?.[0]?.immersion_banner_title_bold ?? "",
    immersion_banner_description:
      immersionData?.data?.[0]?.immersion_banner_description ?? "",
    immersion_banner_button_text:
      immersionData?.data?.[0]?.immersion_banner_button_text ?? "",
    immersion_banner_button_link:
      immersionData?.data?.[0]?.immersion_banner_button_link ?? "",
    immersion_banner_img: immersionData?.data?.[0]?.immersion_banner_img ?? "",

    immersion_banner_img_old:
      immersionData?.data?.[0]?.immersion_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Immersion Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                immersion_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="immersion_banner_title_bold"
                    ...
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="immersion_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="immersion_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Link"
                        type="text"
                        name="immersion_banner_button_link"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateImmersionBanner;
