...
const ModalUpdateImmersionApplyNow = ({ itemEdit, immersionApplyNowData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        immersionApplyNowData?.data?.length
          ? `${apiVersion}/immersion-apply-now/${immersionApplyNowData.data[0].immersion_apply_aid}` // update
          : `${apiVersion}/immersion-apply-now`, // create
        immersionApplyNowData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["immersion-apply-now"] });
      if (!data.success) {
  ...
  const initVal = {
    isUpdateImmersionApplyNow: itemEdit,
    immersion_apply_subtitle:
      immersionApplyNowData?.data?.[0]?.immersion_apply_subtitle ?? "",
    immersion_apply_title:
      immersionApplyNowData?.data?.[0]?.immersion_apply_title ?? "",
    immersion_apply_requirement_list:
      immersionApplyNowData?.data?.[0]?.immersion_apply_requirement_list ?? "",
    immersion_apply_button_text:
      immersionApplyNowData?.data?.[0]?.immersion_apply_button_text ?? "",
    immersion_apply_img:
      immersionApplyNowData?.data?.[0]?.immersion_apply_img ?? "",

    immersion_apply_img_old:
      immersionApplyNowData?.data?.[0]?.immersion_apply_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Immersion Apply Now
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                immersion_apply_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                    ...
                    <div className=" relative">
                      ...
                      <div className="input-wrapper">
                        <InputTextArea
                          label="Requirements"
                          type="text"
                          name="immersion_apply_requirement_list"
                          disabled={mutation.isPending}
                        />
                      </div>
                      <div className="input-wrapper">
                        <InputText
                          label="Button Text"
                          type="text"
                          name="immersion_apply_button_text"
                          disabled={mutation.isPending}
                        />
                      </div>
                    </div>
...
export default ModalUpdateImmersionApplyNow;
