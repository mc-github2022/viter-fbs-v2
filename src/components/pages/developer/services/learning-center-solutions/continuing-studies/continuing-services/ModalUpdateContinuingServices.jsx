...
const ModalUpdateContinuingServices = ({ itemEdit }) => {
  const { store, dispatch } = React.useContext(StoreContext);
  const [animate, setAnimate] = React.useState("translate-x-full");

  const [searchTerm, setSearchTerm] = React.useState(
    itemEdit ? itemEdit.continuing_services_icon : ""
  );
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/continuing-services/${itemEdit.continuing_services_aid}` // update
          : `${apiVersion}/continuing-services`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["continuing-services"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    continuing_services_subtitle_a: itemEdit
      ? itemEdit.continuing_services_subtitle_a
      : "",
    continuing_services_subtitle_b: itemEdit
      ? itemEdit.continuing_services_subtitle_b
      : "",
    continuing_services_title: itemEdit
      ? itemEdit.continuing_services_title
      : "",
    continuing_services_list: itemEdit ? itemEdit.continuing_services_list : "",
    continuing_services_icon: itemEdit ? itemEdit.continuing_services_icon : "",
    continuing_services_prerequisite: itemEdit
      ? itemEdit.continuing_services_prerequisite
      : "",
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Continuing Services
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              const data = {
                ...values,
                continuing_services_icon: selectedIcon,
              };
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Subtitle A"
                        type="text"
                        name="continuing_services_subtitle_a"
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title"
                        type="text"
                        name="continuing_services_title"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Subtitle B"
                        type="text"
                        name="continuing_services_subtitle_b"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Prerequisite"
                        type="text"
                        name="continuing_services_prerequisite"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
...
export default ModalUpdateContinuingServices;
