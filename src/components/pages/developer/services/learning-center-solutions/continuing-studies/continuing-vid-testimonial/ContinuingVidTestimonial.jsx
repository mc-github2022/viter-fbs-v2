...
const ContinuingVidTestimonial = ({
          ...
          <div className="mb-20 text-center relative z-[3]">
            <p className="text-light">
              {continuingTitlesData?.data?.[0]
                ?.continuing_titles_vid_testimonial_subtitle || "Subtitle"}
            </p>
            <h3 className="text-[clamp(20px,7vw,35px)] font-semibold leading-[1.1] text-light">
              {continuingTitlesData?.data?.[0]
                ?.continuing_titles_vid_testimonial_title || "Title"}
            </h3>
            <a
              className="absolute cursor-pointer tooltip-btn -top-4 left-[55rem] "
              data-tooltip="Edit contents"
              onClick={handleUpdateContinuingVidTestimonialTitle}
            >
              <HiPencil className=" bg-[#C7AC27] rounded-full  w-[25px] h-[25px] p-[5px] border-[1px] text-black" />
            </a>
          </div>
          {vidTestimonialData?.data.filter(
            (item) => item.vid_testimonial_category === "Continuing Studies"
          ).length > 3 ? (
            <Slider ref={sliderRef} {...settings}>
              {vidTestimonialData?.data.map((item, key) => {
                if (item.vid_testimonial_category === "Continuing Studies") {
                  ...
                  return (
                    <div
                      className="vidItem addShadow rounded-xl z-[2] max-w-[360px] bg-customGray relative"
                      key={key}
                    >
                      <iframe
...
export default ContinuingVidTestimonial;
