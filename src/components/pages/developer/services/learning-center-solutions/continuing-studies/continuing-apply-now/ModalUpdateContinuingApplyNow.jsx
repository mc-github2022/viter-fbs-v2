...
const ModalUpdateContinuingApplyNow = ({
  itemEdit,
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        continuingApplyNowData?.data?.length
          ? `${apiVersion}/continuing-apply-now/${continuingApplyNowData.data[0].continuing_apply_aid}` // update
          : `${apiVersion}/continuing-apply-now`, // create
        continuingApplyNowData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["continuing-apply-now"] });
      if (!data.success) {
  ...
  const initVal = {
    isUpdateContinuingApplyNow: itemEdit,
    continuing_apply_subtitle:
      continuingApplyNowData?.data?.[0]?.continuing_apply_subtitle ?? "",
    continuing_apply_title:
      continuingApplyNowData?.data?.[0]?.continuing_apply_title ?? "",
    continuing_apply_requirement_list:
      continuingApplyNowData?.data?.[0]?.continuing_apply_requirement_list ??
      "",
    continuing_apply_button_text:
      continuingApplyNowData?.data?.[0]?.continuing_apply_button_text ?? "",
    continuing_apply_other_details:
      continuingApplyNowData?.data?.[0]?.continuing_apply_other_details ?? "",
    continuing_apply_img:
      continuingApplyNowData?.data?.[0]?.continuing_apply_img ?? "",

    continuing_apply_img_old:
      continuingApplyNowData?.data?.[0]?.continuing_apply_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Continuing Apply Now
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                continuing_apply_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                    ...
                    <div className=" relative">
                      <div className="input-wrapper">
                        <InputText
                          label="Subtitle"
                          type="text"
                          name="continuing_apply_subtitle"
                      ...
                      <div className="input-wrapper">
                        <InputTextArea
                          label="Requirements"
                          type="text"
                          name="continuing_apply_requirement_list"
                          disabled={mutation.isPending}
                        />
                      </div>
                      <div className="input-wrapper">
                        <InputTextArea
                          label="Other Details"
                          type="text"
                          name="continuing_apply_other_details"
                          disabled={mutation.isPending}
                        />
                      </div>
                      <div className="input-wrapper">
                        <InputText
                          label="Button Text"
                          type="text"
                          name="continuing_apply_button_text"
                          disabled={mutation.isPending}
                        />
                      </div>
                    </div>
                    <div className="input-wrapper h-[475px] overflow-auto">
                            ...
                            Array.from(photoArrayList).map((item, key) => {
                              ...
                              return (
                                <React.Fragment key={key}>
                                  <li
                                    className={`relative z-10 h-[300px] w-[300px] group cursor-pointer overflow-hidden ${
...
export default ModalUpdateContinuingApplyNow;
