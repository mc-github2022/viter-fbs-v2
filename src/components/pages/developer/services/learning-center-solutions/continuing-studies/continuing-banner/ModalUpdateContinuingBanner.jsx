...
const ModalUpdateContinuingBanner = ({ itemEdit, continuingData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        continuingData?.data?.length
          ? `${apiVersion}/continuing/${continuingData.data[0].continuing_banner_aid}` // update
          : `${apiVersion}/continuing`, // create
        continuingData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["continuing"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    isUpdateContinuing: itemEdit,
    continuing_banner_title:
      continuingData?.data?.[0]?.continuing_banner_title ?? "",
    continuing_banner_title_bold:
      continuingData?.data?.[0]?.continuing_banner_title_bold ?? "",
    continuing_banner_description:
      continuingData?.data?.[0]?.continuing_banner_description ?? "",
    continuing_banner_button_text:
      continuingData?.data?.[0]?.continuing_banner_button_text ?? "",
    continuing_banner_button_link:
      continuingData?.data?.[0]?.continuing_banner_button_link ?? "",
    continuing_banner_img:
      continuingData?.data?.[0]?.continuing_banner_img ?? "",

    continuing_banner_img_old:
      continuingData?.data?.[0]?.continuing_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Continuing Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                continuing_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="continuing_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    ...
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="continuing_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="continuing_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Link"
                        type="text"
                        name="continuing_banner_button_link"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateContinuingBanner;
