...
const ModalUpdateOjtOverview = ({ itemEdit, ojtOverviewData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        ojtOverviewData?.data?.length
          ? `${apiVersion}/ojt-overview/${ojtOverviewData.data[0].ojt_overview_aid}` // update
          : `${apiVersion}/ojt-overview`, // create
        ojtOverviewData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["ojt-overview"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    ojtOverviewUpdate: itemEdit,
    ojt_overview_subtitle:
      ojtOverviewData?.data?.[0]?.ojt_overview_subtitle ?? "",
    ojt_overview_title: ojtOverviewData?.data?.[0]?.ojt_overview_title ?? "",
    ojt_overview_button_text:
      ojtOverviewData?.data?.[0]?.ojt_overview_button_text ?? "",
    ojt_overview_img: ojtOverviewData?.data?.[0]?.ojt_overview_img ?? "",
    ojt_overview_file: ojtOverviewData?.data?.[0]?.ojt_overview_file ?? "",

    ojt_overview_img_old: ojtOverviewData?.data?.[0]?.ojt_overview_img ?? "",
    ojt_overview_file_old: ojtOverviewData?.data?.[0]?.ojt_overview_file ?? "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} OJT Overview</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                ojt_overview_img: logoImages.map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                ),
                ojt_overview_file: clientImages.map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateOjtOverview;
