...
const ModalUpdateWebsiteBanner = ({ itemEdit, websiteData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        websiteData?.data?.length
          ? `${apiVersion}/website/${websiteData.data[0].website_banner_aid}` // update
          : `${apiVersion}/website`, // create
        websiteData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateWebsite: itemEdit,
    website_banner_title: websiteData?.data?.[0]?.website_banner_title ?? "",
    website_banner_title_bold:
      websiteData?.data?.[0]?.website_banner_title_bold ?? "",
    website_banner_description:
      websiteData?.data?.[0]?.website_banner_description ?? "",
    website_banner_button_text:
      websiteData?.data?.[0]?.website_banner_button_text ?? "",
    website_banner_img: websiteData?.data?.[0]?.website_banner_img ?? "",

    website_banner_img_old: websiteData?.data?.[0]?.website_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Web Design Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                website_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="website_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="website_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="website_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateWebsiteBanner;
