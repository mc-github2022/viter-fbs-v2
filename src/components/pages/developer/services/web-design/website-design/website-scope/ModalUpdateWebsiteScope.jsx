...
const ModalUpdateWebsiteScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/website-scope/${itemEdit.website_scope_aid}` // update
          : `${apiVersion}/website-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["website-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    website_scope_title: itemEdit ? itemEdit.website_scope_title : "",
    website_scope_desc: itemEdit ? itemEdit.website_scope_desc : "",
    website_scope_button_text: itemEdit
      ? itemEdit.website_scope_button_text
      : "",
    website_scope_img: itemEdit ? itemEdit.website_scope_img : "",

    website_scope_img_old: itemEdit ? itemEdit.website_scope_img : "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Web Design Scope
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                website_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="website_scope_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                  </div>
...
export default ModalUpdateWebsiteScope;
