...
const WordpressBanner = ({
  ...
  return (
    <>
      ...
      >
        {wordpressData?.data?.length > 0 && wordpressBannerImage?.length > 0 ? (
          <>
            {wordpressBannerImage.map((img, index) => (
              <LoadImages
                url={`${googleHDViewLink}${img?.id}`}
                alt={`wordpress banner ${index + 1}`}
          ...
          <div className="wrapper flex flex-col justify-center items-center h-full text-center lg:grid lg:grid-cols-2">
            <div className="text-center py-10 lg:text-left">
              ...
              <a
                ...
                onClick={handleUpdateWordpressBanner}
              >
