...
const ModalUpdateWordpressBanner = ({ itemEdit, wordpressData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        wordpressData?.data?.length
          ? `${apiVersion}/wordpress/${wordpressData.data[0].wordpress_banner_aid}` // update
          : `${apiVersion}/wordpress`, // create
        wordpressData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateWordpress: itemEdit,
    wordpress_banner_title:
      wordpressData?.data?.[0]?.wordpress_banner_title ?? "",
    wordpress_banner_title_bold:
      wordpressData?.data?.[0]?.wordpress_banner_title_bold ?? "",
    wordpress_banner_description:
      wordpressData?.data?.[0]?.wordpress_banner_description ?? "",
    wordpress_banner_button_text:
      wordpressData?.data?.[0]?.wordpress_banner_button_text ?? "",
    wordpress_banner_img: wordpressData?.data?.[0]?.wordpress_banner_img ?? "",

    wordpress_banner_img_old:
      wordpressData?.data?.[0]?.wordpress_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} WordPress Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                wordpress_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="wordpress_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="wordpress_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="wordpress_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateWordpressBanner;
