...
const ModalUpdateWordpressScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/wordpress-scope/${itemEdit.wordpress_scope_aid}` // update
          : `${apiVersion}/wordpress-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["wordpress-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    wordpress_scope_title: itemEdit ? itemEdit.wordpress_scope_title : "",
    wordpress_scope_desc: itemEdit ? itemEdit.wordpress_scope_desc : "",
    wordpress_scope_button_text: itemEdit
      ? itemEdit.wordpress_scope_button_text
      : "",
    wordpress_scope_img: itemEdit ? itemEdit.wordpress_scope_img : "",

    wordpress_scope_img_old: itemEdit ? itemEdit.wordpress_scope_img : "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} WordPress Scope
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                wordpress_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="wordpress_scope_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                  </div>
...
export default ModalUpdateWordpressScope;
