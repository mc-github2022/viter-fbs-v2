...
const SinglepageBanner = ({
  ...
  return (
    <>
      ...
      >
        {singlepageData?.data?.length > 0 &&
        singlepageBannerImage?.length > 0 ? (
          <>
            {singlepageBannerImage.map((img, index) => (
              <LoadImages
                url={`${googleHDViewLink}${img?.id}`}
                alt={`singlepage banner ${index + 1}`}
