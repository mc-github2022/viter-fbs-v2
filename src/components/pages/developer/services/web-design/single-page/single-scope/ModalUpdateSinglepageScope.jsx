...
const ModalUpdateSinglepageScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/singlepage-scope/${itemEdit.singlepage_scope_aid}` // update
          : `${apiVersion}/singlepage-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["singlepage-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    singlepage_scope_title: itemEdit ? itemEdit.singlepage_scope_title : "",
    singlepage_scope_desc: itemEdit ? itemEdit.singlepage_scope_desc : "",
    singlepage_scope_button_text: itemEdit
      ? itemEdit.singlepage_scope_button_text
      : "",
    singlepage_scope_img: itemEdit ? itemEdit.singlepage_scope_img : "",

    singlepage_scope_img_old: itemEdit ? itemEdit.singlepage_scope_img : "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Single Page Scope
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                singlepage_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="singlepage_scope_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                  </div>
...
export default ModalUpdateSinglepageScope;
