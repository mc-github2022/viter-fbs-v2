...
const SinglepageOverview = ({
  ...
  const singlepageOverviewImage = getConvertStringToJSONparseData(
    singlepageOverviewData?.data?.[0]?.singlepage_overview_img
  );

  const singlepageOverviewFile = getConvertStringToJSONparseData(
      ...
      <section className="ServiceHrOverview py-20 bg-customGray">
        <div className="customContainer relative">
          <a
            ...
            onClick={handleUpdateSinglepageOverview}
          >
          ...
          <div className="wrapper lg:grid lg:grid-cols-2 gap-12">
            <div className="relative">
              {singlepageOverviewData?.data?.length > 0 &&
              singlepageOverviewImage?.length > 0 ? (
                <>
                  {singlepageOverviewImage.map((img, index) => (
                    <LoadImages
              ...
              <div className="flex flex-col md:flex md:flex-row items-center gap-12">
                ...
                >
                  {singlepageOverviewData?.data?.[0]
                    ?.singlepage_overview_button_text || "Button"}
                </button>

                {singlepageOverviewFile.map((file, index) => (
                  <a
            ...
            <div className="mt-14 lg:mt-0">
              <a
                ...
                data-tooltip="Edit contents"
                onClick={handleUpdateSinglepageOverviewList}
              >
                  ...
                  <div className="flex gap-4 items-start">
                    ...
                    <div>
                      <h3 className="text-primary bg-gradient-to-r from-primary to-secondary bg-clip-text text-[transparent] text-[clamp(16px,5vw,24px)] mb-3">
                        {singlepageOverviewData?.data?.[0]
                          ?.singlepage_overview_list_title_a || "Title A"}
                      </h3>
                ...
                <li className="flex items-start gap-4 text-dark mb-6">
                  <div className="flex gap-4 items-start">
                    ...
                    <div>
                      <h3 className="text-primary bg-gradient-to-r from-primary to-secondary bg-clip-text text-[transparent] text-[clamp(16px,5vw,24px)] mb-3">
                        {singlepageOverviewData?.data?.[0]
                          ?.singlepage_overview_list_title_b || "Title B"}
                      </h3>
                      <p className="text-justify">
                        {singlepageOverviewData?.data?.[0]
                ...
                <li className="flex items-start gap-4 text-dark">
                  <div className="flex gap-4 items-start">
                    ...
                    <div>
                      <h3 className="text-primary bg-gradient-to-r from-primary to-secondary bg-clip-text text-[transparent] text-[clamp(16px,5vw,24px)] mb-3">
                        {singlepageOverviewData?.data?.[0]
                          ?.singlepage_overview_list_title_c || "Title C"}
                      </h3>
                      <p className="text-justify">
                        {singlepageOverviewData?.data?.[0]
