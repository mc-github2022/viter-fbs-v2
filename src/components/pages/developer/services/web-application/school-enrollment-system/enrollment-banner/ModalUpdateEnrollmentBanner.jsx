...
const ModalUpdateEnrollmentBanner = ({ itemEdit, enrollmentData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        enrollmentData?.data?.length
          ? `${apiVersion}/enrollment/${enrollmentData.data[0].enrollment_banner_aid}` // update
          : `${apiVersion}/enrollment`, // create
        enrollmentData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateEnrollment: itemEdit,
    enrollment_banner_title:
      enrollmentData?.data?.[0]?.enrollment_banner_title ?? "",
    enrollment_banner_title_bold:
      enrollmentData?.data?.[0]?.enrollment_banner_title_bold ?? "",
    enrollment_banner_description:
      enrollmentData?.data?.[0]?.enrollment_banner_description ?? "",
    enrollment_banner_button_text:
      enrollmentData?.data?.[0]?.enrollment_banner_button_text ?? "",
    enrollment_banner_img:
      enrollmentData?.data?.[0]?.enrollment_banner_img ?? "",

    enrollment_banner_img_old:
      enrollmentData?.data?.[0]?.enrollment_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Enrollment Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                enrollment_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="enrollment_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="enrollment_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="enrollment_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateEnrollmentBanner;
