...
const ModalUpdateEnrollmentScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/enrollment-scope/${itemEdit.enrollment_scope_aid}` // update
          : `${apiVersion}/enrollment-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["enrollment-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    enrollment_scope_title: itemEdit ? itemEdit.enrollment_scope_title : "",
    enrollment_scope_desc: itemEdit ? itemEdit.enrollment_scope_desc : "",
    enrollment_scope_button_text: itemEdit
      ? itemEdit.enrollment_scope_button_text
      : "",
    enrollment_scope_img: itemEdit ? itemEdit.enrollment_scope_img : "",

    enrollment_scope_img_old: itemEdit ? itemEdit.enrollment_scope_img : "",
    pendingDeleteFile: [],
  };
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                enrollment_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="enrollment_scope_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                  </div>
...
export default ModalUpdateEnrollmentScope;
