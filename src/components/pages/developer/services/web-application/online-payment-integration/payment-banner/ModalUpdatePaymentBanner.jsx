...
const ModalUpdatePaymentBanner = ({ itemEdit, paymentData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        paymentData?.data?.length
          ? `${apiVersion}/payment/${paymentData.data[0].payment_banner_aid}` // update
          : `${apiVersion}/payment`, // create
        paymentData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdatePayment: itemEdit,
    payment_banner_title: paymentData?.data?.[0]?.payment_banner_title ?? "",
    payment_banner_title_bold:
      paymentData?.data?.[0]?.payment_banner_title_bold ?? "",
    payment_banner_description:
      paymentData?.data?.[0]?.payment_banner_description ?? "",
    payment_banner_button_text:
      paymentData?.data?.[0]?.payment_banner_button_text ?? "",
    payment_banner_img: paymentData?.data?.[0]?.payment_banner_img ?? "",

    payment_banner_img_old: paymentData?.data?.[0]?.payment_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Payment Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                payment_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="payment_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="payment_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="payment_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdatePaymentBanner;
