...
const ModalUpdatePaymentScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/payment-scope/${itemEdit.payment_scope_aid}` // update
          : `${apiVersion}/payment-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["payment-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    payment_scope_title: itemEdit ? itemEdit.payment_scope_title : "",
    payment_scope_desc: itemEdit ? itemEdit.payment_scope_desc : "",
    payment_scope_button_text: itemEdit
      ? itemEdit.payment_scope_button_text
      : "",
    payment_scope_img: itemEdit ? itemEdit.payment_scope_img : "",

    payment_scope_img_old: itemEdit ? itemEdit.payment_scope_img : "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} Payment Scope</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                payment_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="payment_scope_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                  </div>
...
export default ModalUpdatePaymentScope;
