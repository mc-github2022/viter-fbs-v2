...
const ModalUpdateHrisScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/hris-scope/${itemEdit.hris_scope_aid}` // update
          : `${apiVersion}/hris-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["hris-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  React.useEffect(() => {
    setAnimate("");
    if (itemEdit) {
      const photos = getConvertStringToJSONparseData(itemEdit.hris_scope_img);
      setPhotoArrayList(photos);
    }
  }, []);

  const initVal = {
    hris_scope_title: itemEdit ? itemEdit.hris_scope_title : "",
    hris_scope_desc: itemEdit ? itemEdit.hris_scope_desc : "",
    hris_scope_img: itemEdit ? itemEdit.hris_scope_img : "",

    hris_scope_img_old: itemEdit ? itemEdit.hris_scope_img : "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} HRIS Scope</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                hris_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateHrisScope;
