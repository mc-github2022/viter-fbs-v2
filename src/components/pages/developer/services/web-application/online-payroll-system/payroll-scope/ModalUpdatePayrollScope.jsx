...
const ModalUpdatePayrollScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/payroll-scope/${itemEdit.payroll_scope_aid}` // update
          : `${apiVersion}/payroll-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["payroll-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    payroll_scope_title: itemEdit ? itemEdit.payroll_scope_title : "",
    payroll_scope_desc: itemEdit ? itemEdit.payroll_scope_desc : "",
    payroll_scope_img: itemEdit ? itemEdit.payroll_scope_img : "",

    payroll_scope_img_old: itemEdit ? itemEdit.payroll_scope_img : "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} Payroll Scope</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                payroll_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdatePayrollScope;
