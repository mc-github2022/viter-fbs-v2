...
const ModalUpdatePayrollBanner = ({ itemEdit, payrollData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        payrollData?.data?.length
          ? `${apiVersion}/payroll/${payrollData.data[0].payroll_banner_aid}` // update
          : `${apiVersion}/payroll`, // create
        payrollData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdatePayroll: itemEdit,
    payroll_banner_title: payrollData?.data?.[0]?.payroll_banner_title ?? "",
    payroll_banner_title_bold:
      payrollData?.data?.[0]?.payroll_banner_title_bold ?? "",
    payroll_banner_description:
      payrollData?.data?.[0]?.payroll_banner_description ?? "",
    payroll_banner_button_text:
      payrollData?.data?.[0]?.payroll_banner_button_text ?? "",
    payroll_banner_button_link:
      payrollData?.data?.[0]?.payroll_banner_button_link ?? "",
    payroll_banner_img: payrollData?.data?.[0]?.payroll_banner_img ?? "",

    payroll_banner_img_old: payrollData?.data?.[0]?.payroll_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Payroll Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                payroll_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="payroll_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="payroll_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
...
export default ModalUpdatePayrollBanner;
