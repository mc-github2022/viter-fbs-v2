...
const ModalUpdateBookingBanner = ({ itemEdit, bookingData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        bookingData?.data?.length
          ? `${apiVersion}/booking/${bookingData.data[0].booking_banner_aid}` // update
          : `${apiVersion}/booking`, // create
        bookingData?.data?.length ? "put" : "post",
        values,
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateBooking: itemEdit,
    booking_banner_title: bookingData?.data?.[0]?.booking_banner_title ?? "",
    booking_banner_title_bold:
      bookingData?.data?.[0]?.booking_banner_title_bold ?? "",
    booking_banner_description:
      bookingData?.data?.[0]?.booking_banner_description ?? "",
    booking_banner_button_text:
      bookingData?.data?.[0]?.booking_banner_button_text ?? "",
    booking_banner_button_link:
      bookingData?.data?.[0]?.booking_banner_button_link ?? "",
    booking_banner_img: bookingData?.data?.[0]?.booking_banner_img ?? "",

    booking_banner_img_old: bookingData?.data?.[0]?.booking_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Booking Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                booking_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  }),
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="booking_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="booking_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="booking_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Link"
                        type="text"
                        name="booking_banner_button_link"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateBookingBanner;
