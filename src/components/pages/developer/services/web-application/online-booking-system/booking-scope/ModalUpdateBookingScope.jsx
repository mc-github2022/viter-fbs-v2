...
const ModalUpdateBookingScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/booking-scope/${itemEdit.booking_scope_aid}` // update
          : `${apiVersion}/booking-scope`, // create
        itemEdit ? "put" : "post",
        values,
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["booking-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    booking_scope_title: itemEdit ? itemEdit.booking_scope_title : "",
    booking_scope_desc: itemEdit ? itemEdit.booking_scope_desc : "",
    booking_scope_button_text: itemEdit
      ? itemEdit.booking_scope_button_text
      : "",
    booking_scope_img: itemEdit ? itemEdit.booking_scope_img : "",

    booking_scope_img_old: itemEdit ? itemEdit.booking_scope_img : "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} Booking Scope</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                booking_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  }),
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="booking_scope_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                  </div>
...
export default ModalUpdateBookingScope;
