...
const ModalUpdateAssetBanner = ({ itemEdit, assetData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        assetData?.data?.length
          ? `${apiVersion}/asset/${assetData.data[0].asset_banner_aid}` // update
          : `${apiVersion}/asset`, // create
        assetData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateAsset: itemEdit,
    asset_banner_title: assetData?.data?.[0]?.asset_banner_title ?? "",
    asset_banner_title_bold:
      assetData?.data?.[0]?.asset_banner_title_bold ?? "",
    asset_banner_description:
      assetData?.data?.[0]?.asset_banner_description ?? "",
    asset_banner_button_text:
      assetData?.data?.[0]?.asset_banner_button_text ?? "",
    asset_banner_button_link:
      assetData?.data?.[0]?.asset_banner_button_link ?? "",
    asset_banner_img: assetData?.data?.[0]?.asset_banner_img ?? "",

    asset_banner_img_old: assetData?.data?.[0]?.asset_banner_img ?? "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} Asset Banner</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                asset_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateAssetBanner;
