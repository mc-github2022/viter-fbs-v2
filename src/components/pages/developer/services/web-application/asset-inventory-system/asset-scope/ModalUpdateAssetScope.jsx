...
const ModalUpdateAssetScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/asset-scope/${itemEdit.asset_scope_aid}` // update
          : `${apiVersion}/asset-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["asset-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  React.useEffect(() => {
    setAnimate("");
    if (itemEdit) {
      const photos = getConvertStringToJSONparseData(itemEdit.asset_scope_img);
      setPhotoArrayList(photos);
    }
  }, []);

  const initVal = {
    asset_scope_title: itemEdit ? itemEdit.asset_scope_title : "",
    asset_scope_desc: itemEdit ? itemEdit.asset_scope_desc : "",
    asset_scope_button_text: itemEdit ? itemEdit.asset_scope_button_text : "",
    asset_scope_img: itemEdit ? itemEdit.asset_scope_img : "",

    asset_scope_img_old: itemEdit ? itemEdit.asset_scope_img : "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} Asset Scope</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                asset_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateAssetScope;
