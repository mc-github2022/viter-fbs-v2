...
const ModalUpdateDonationScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/donation-scope/${itemEdit.donation_scope_aid}` // update
          : `${apiVersion}/donation-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["donation-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    donation_scope_title: itemEdit ? itemEdit.donation_scope_title : "",
    donation_scope_desc: itemEdit ? itemEdit.donation_scope_desc : "",
    donation_scope_button_text: itemEdit
      ? itemEdit.donation_scope_button_text
      : "",
    donation_scope_img: itemEdit ? itemEdit.donation_scope_img : "",

    donation_scope_img_old: itemEdit ? itemEdit.donation_scope_img : "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Donation Scope
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                donation_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="form-input ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="donation_scope_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                  </div>
...
export default ModalUpdateDonationScope;
