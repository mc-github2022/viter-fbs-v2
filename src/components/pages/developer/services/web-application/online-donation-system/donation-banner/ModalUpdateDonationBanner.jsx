...
const ModalUpdateDonationBanner = ({ itemEdit, donationData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        donationData?.data?.length
          ? `${apiVersion}/donation/${donationData.data[0].donation_banner_aid}` // update
          : `${apiVersion}/donation`, // create
        donationData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateDonation: itemEdit,
    donation_banner_title: donationData?.data?.[0]?.donation_banner_title ?? "",
    donation_banner_title_bold:
      donationData?.data?.[0]?.donation_banner_title_bold ?? "",
    donation_banner_description:
      donationData?.data?.[0]?.donation_banner_description ?? "",
    donation_banner_button_text:
      donationData?.data?.[0]?.donation_banner_button_text ?? "",
    donation_banner_img: donationData?.data?.[0]?.donation_banner_img ?? "",

    donation_banner_img_old: donationData?.data?.[0]?.donation_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Donation Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                donation_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="donation_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="donation_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="donation_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateDonationBanner;
