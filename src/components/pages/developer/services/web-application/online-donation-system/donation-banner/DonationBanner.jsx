...
const DonationBanner = ({
  ...
  return (
    <>
      ...
      >
        {donationData?.data?.length > 0 && donationBannerImage?.length > 0 ? (
          <>
            {donationBannerImage.map((img, index) => (
              <LoadImages
                url={`${googleHDViewLink}${img?.id}`}
                alt={`donation banner ${index + 1}`}
