...
const ModalUpdateMarketingBanner = ({ itemEdit, marketingData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        marketingData?.data?.length
          ? `${apiVersion}/marketing/${marketingData.data[0].marketing_banner_aid}` // update
          : `${apiVersion}/marketing`, // create
        marketingData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    ...
    marketing_banner_button_text:
      marketingData?.data?.[0]?.marketing_banner_button_text ?? "",
    marketing_banner_img: marketingData?.data?.[0]?.marketing_banner_img ?? "",

    marketing_banner_img_old:
      marketingData?.data?.[0]?.marketing_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Marketing Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                marketing_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="marketing_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="marketing_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
...
export default ModalUpdateMarketingBanner;
