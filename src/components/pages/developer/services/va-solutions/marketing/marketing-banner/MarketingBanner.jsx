...
const MarketingBanner = ({
  ...
  return (
    <>
      ...
      >
        {marketingData?.data?.length > 0 && marketingBannerImage?.length > 0 ? (
          <>
            {marketingBannerImage.map((img, index) => (
              <LoadImages
                url={`${googleHDViewLink}${img?.id}`}
                alt={`marketing banner ${index + 1}`}
