...
const SocialMediaServices = ({
  ...
  handleUpdateSocialServicesList,
  isFetchingServices,
  isLoadingServices,
  error,
  setItemEdit,
  ...
  const handleDelete = (item) => {
    ...
    setIsData(item.social_services_list_title);
    setIsId(item.social_services_list_aid);
  };
    ...
    <>
        ...
        <div className="customContainer relative">
          <div className="sectionDesc text-center md:w-[70%] mx-auto mb-14">
            <h2 className="text-[clamp(20px,6vw,40px)] leading-[1.1] font-semibold mb-10">
              {socialServicesTitleData?.data?.[0]
                ?.social_services_title_black_a || "Title"}{" "}
              <span className="text-primary">
                {socialServicesTitleData?.data?.[0]
                  ?.social_services_title_highlighted || "Title"}{" "}
              </span>{" "}
              <br />
              {socialServicesTitleData?.data?.[0]
                ?.social_services_title_black_b || "Title"}
            </h2>
            <p className="subDesc mb-10">
              {socialServicesTitleData?.data?.[0]
                ?.social_services_title_description || "Description"}
            </p>
            <button
              onClick={handleOpen}
            ...
            >
              {socialServicesTitleData?.data?.[0]
                ?.social_services_title_button_text || "Button"}
            </button>
            <a
              className="absolute cursor-pointer tooltip-btn right-[10rem] -top-6 "
              data-tooltip="Edit contents"
              onClick={handleUpdateSocialServicesTitle}
            >
          ...
          <a
            ...
            onClick={handleUpdateSocialServicesList}
          >
          ...
          {(isLoadingServices || socialServicesData?.data?.length === 0) && (
          ...
          <ul className="serviceInclusion grid md:grid-cols-2 gap-6">
            {isLoadingServices || isFetchingServices ? (
              ...
              socialServicesData?.data.map((item, key) => {
                const SelectedIcon = item.social_services_list_icon
                  ? icons[item.social_services_list_icon]
                  : null;

                return (
                  <li className="flex items-center gap-7" key={key}>
                    {item.social_services_list_title
                      .split("\n") // Split by new lines
      ...
      {store.isDelete && (
        <ModalDelete
          ...
          mysqlEndpoint={`/v1/social-services-list/${id}`}
          item={isData}
        />
      )}
    </>
