...
const ModalUpdateSocialMediaBanner = ({ itemEdit, socialData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        socialData?.data?.length
          ? `${apiVersion}/social/${socialData.data[0].social_banner_aid}` // update
          : `${apiVersion}/social`, // create
        socialData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateSocial: itemEdit,
    social_banner_title: socialData?.data?.[0]?.social_banner_title ?? "",
    social_banner_title_bold:
      socialData?.data?.[0]?.social_banner_title_bold ?? "",
    social_banner_description:
      socialData?.data?.[0]?.social_banner_description ?? "",
    social_banner_button_text:
      socialData?.data?.[0]?.social_banner_button_text ?? "",
    social_banner_img: socialData?.data?.[0]?.social_banner_img ?? "",

    social_banner_img_old: socialData?.data?.[0]?.social_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Social Media Management Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                social_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="social_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="social_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateSocialMediaBanner;
