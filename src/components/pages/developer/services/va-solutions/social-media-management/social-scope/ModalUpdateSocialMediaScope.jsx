...
const ModalUpdateSocialMediaScope = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/social-scope/${itemEdit.social_scope_aid}` // update
          : `${apiVersion}/social-scope`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["social-scope"] });
      if (!data.success) {
        console.log("Error");
  ...
  React.useEffect(() => {
    setAnimate("");
    if (itemEdit) {
      const photos = getConvertStringToJSONparseData(itemEdit.social_scope_img);
      setPhotoArrayList(photos);
    }
  }, []);

  const initVal = {
    social_scope_title: itemEdit ? itemEdit.social_scope_title : "",
    social_scope_desc: itemEdit ? itemEdit.social_scope_desc : "",
    social_scope_button_text: itemEdit ? itemEdit.social_scope_button_text : "",
    social_scope_img: itemEdit ? itemEdit.social_scope_img : "",

    social_scope_img_old: itemEdit ? itemEdit.social_scope_img : "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Social Media Management Scope
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                social_scope_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateSocialMediaScope;
