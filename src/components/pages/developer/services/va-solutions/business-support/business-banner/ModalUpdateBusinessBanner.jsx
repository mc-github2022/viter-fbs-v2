...
const ModalUpdateBusinessBanner = ({ itemEdit, businessData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        businessData?.data?.length
          ? `${apiVersion}/business/${businessData.data[0].business_banner_aid}` // update
          : `${apiVersion}/business`, // create
        businessData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateBusiness: itemEdit,
    business_banner_title: businessData?.data?.[0]?.business_banner_title ?? "",
    business_banner_title_bold:
      businessData?.data?.[0]?.business_banner_title_bold ?? "",
    business_banner_description:
      businessData?.data?.[0]?.business_banner_description ?? "",
    business_banner_button_text:
      businessData?.data?.[0]?.business_banner_button_text ?? "",
    business_banner_img: businessData?.data?.[0]?.business_banner_img ?? "",

    business_banner_img_old: businessData?.data?.[0]?.business_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Business Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                business_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="business_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="business_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="business_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateBusinessBanner;
