...
const ModalUpdateBusinessServicesList = ({ itemEdit }) => {
  const { store, dispatch } = React.useContext(StoreContext);
  const [animate, setAnimate] = React.useState("translate-x-full");

  const [searchTerm, setSearchTerm] = React.useState(
    itemEdit ? itemEdit.business_services_list_icon : ""
  );
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${apiVersion}/business-services-list/${itemEdit.business_services_list_aid}` // update
          : `${apiVersion}/business-services-list`, // create
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: ["business-services-list"],
      });
      if (!data.success) {
        console.log("Error");
  ...
  const initVal = {
    business_services_list_title: itemEdit
      ? itemEdit.business_services_list_title
      : "",
    business_services_list_icon: itemEdit
      ? itemEdit.business_services_list_icon
      : "",
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Business Services List
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              const data = {
                ...values,
                business_services_list_icon: selectedIcon,
              };
                  ...
                  <div className="form-input ">
                    <div className="input-wrapper" ref={refSearch}>
                      <InputText
                        label="Search Icon"
                        type="text"
                        name="business_services_list_icon"
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title"
                        type="text"
                        name="business_services_list_title"
...
export default ModalUpdateBusinessServicesList;
