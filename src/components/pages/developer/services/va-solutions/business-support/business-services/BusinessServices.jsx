...
const BusinessServices = ({
  ...
  const handleDelete = (item) => {
    ...
    setIsData(item.business_services_list_title);
    setIsId(item.business_services_list_aid);
  };
    ...
    <>
      <section className="serviceList pt-0 pb-20 lg:pt-10">
        <div className="customContainer relative">
          <div className="sectionDesc text-center md:w-[70%] mx-auto mb-14">
            <h2 className="text-[clamp(20px,6vw,40px)] leading-[1.1] font-semibold mb-10">
              {businessServicesTitleData?.data?.[0]
                ?.business_services_title_black_a || "Title"}{" "}
            ...
            <p className="subDesc mb-10">
              {businessServicesTitleData?.data?.[0]
                ?.business_services_title_description || "Description"}
            </p>
            <button
              onClick={handleOpen}
            ...
            >
              {businessServicesTitleData?.data?.[0]
                ?.business_services_title_button_text || "Button"}
            </button>
            <a
              className="absolute cursor-pointer tooltip-btn right-[10rem] -top-6 "
              data-tooltip="Edit contents"
              onClick={handleUpdateBusinessServicesTitle}
            >
          ...
          <a
            ...
            onClick={handleUpdateBusinessServicesList}
          >
          ...
          {(isLoadingServices || businessServicesData?.data?.length === 0) && (
          ...
          <ul className="serviceInclusion grid md:grid-cols-2 gap-6">
            {isLoadingServices || isFetchingServices ? (
              ...
              businessServicesData?.data.map((item, key) => {
                const SelectedIcon = item.business_services_list_icon
                  ? icons[item.business_services_list_icon]
                  : null;

                return (
                  <li className="flex items-center gap-7" key={key}>
                    {item.business_services_list_title
                      .split("\n") // Split by new lines
      ...
      {store.isDelete && (
        <ModalDelete
          ...
          mysqlEndpoint={`/v1/business-services-list/${id}`}
          item={isData}
        />
      )}
    </>
