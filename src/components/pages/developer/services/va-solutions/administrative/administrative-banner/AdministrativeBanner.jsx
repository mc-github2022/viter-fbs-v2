  ...
  return (
    <>
      ...
      >
        ...
        administrativeBannerImage?.length > 0 ? (
          <>
            {administrativeBannerImage.map((img, index) => (
              <LoadImages
                url={`${googleHDViewLink}${img?.id}`}
                alt={`administrative banner ${index + 1}`}
            ...
            <div className="text-center py-10 lg:text-left">
              ...
              {administrativeData?.data.map((item, key) => (
                <button
                  ...
                  key={key}
                >
                  {item.administrative_banner_button_text}
                </button>
              ))}
              <a
                className="absolute cursor-pointer tooltip-btn left-[42rem] top-2"
                data-tooltip="Edit contents"
                onClick={handleUpdateAdministrativeBanner}
              >
...
export default AdministrativeBanner;
