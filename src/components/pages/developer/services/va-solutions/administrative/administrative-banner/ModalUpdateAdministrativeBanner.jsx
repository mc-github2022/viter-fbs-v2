...
const ModalUpdateAdministrativeBanner = ({ itemEdit, administrativeData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        administrativeData?.data?.length
          ? `${apiVersion}/administrative/${administrativeData.data[0].administrative_banner_aid}` // update
          : `${apiVersion}/administrative`, // create
        administrativeData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["administrative"] });
      if (!data.success) {
        console.log("Error");
  ...
  React.useEffect(() => {
    ...
    if (administrativeData) {
      const photos = getConvertStringToJSONparseData(
        administrativeData?.data?.[0]?.administrative_banner_img
      );
  ...
  const initVal = {
    isUpdateAdministrative: itemEdit,
    administrative_banner_title:
    ...
    administrative_banner_button_text:
      administrativeData?.data?.[0]?.administrative_banner_button_text ?? "",
    administrative_banner_img:
      administrativeData?.data?.[0]?.administrative_banner_img ?? "",

    administrative_banner_img_old:
      administrativeData?.data?.[0]?.administrative_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Administrative Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                administrative_banner_img: Array.from(photoArrayList).map(
                  (item) =>
                    JSON.stringify({
                      name: item.name,
                      id: item?.id || "",
                  ...
                  <div className="pr-2 ">
                    ...
                    <div className="input-wrapper">
                      <InputTextArea
                        label="Title"
                        type="text"
                        name="administrative_banner_title"
                    ...
                    <div className="input-wrapper">
                      <InputText
                        label="Title Bold"
                        type="text"
                        name="administrative_banner_title_bold"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper ">
                      <InputTextArea
                        label="Description"
                        type="text"
                        name="administrative_banner_description"
                        className="h-[400px]"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div className="input-wrapper">
                      <InputText
                        label="Button"
                        type="text"
                        name="administrative_banner_button_text"
                        disabled={mutation.isPending}
                      />
                    </div>
                    <div
...
export default ModalUpdateAdministrativeBanner;
