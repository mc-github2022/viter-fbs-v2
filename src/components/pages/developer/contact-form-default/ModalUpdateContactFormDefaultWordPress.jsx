...
const ModalUpdateContactFormDefaultWordPress = ({
  itemEdit,
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        contactFormWordpressData?.data?.length
          ? `${apiVersion}/contactWordpress/${contactFormWordpressData.data[0].form_wordpress_aid}` // update
          : `${apiVersion}/contactWordpress`, // create
        contactFormWordpressData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["contactWordpress"] });
      if (!data.success) {
  ...
  const initVal = {
    isUpdateContactFormWordpress: itemEdit,
    form_wordpress_file:
      contactFormWordpressData?.data?.[0]?.form_wordpress_file ?? "",

    form_wordpress_file_old:
      contactFormWordpressData?.data?.[0]?.form_wordpress_file ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Contact Form (Wordpress)
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                form_wordpress_file: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateContactFormDefaultWordPress;
