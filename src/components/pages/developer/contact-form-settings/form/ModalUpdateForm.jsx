...
const ModalUpdateForm = ({ itemEdit, contactFormData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        contactFormData?.data?.length
          ? `${apiVersion}/contactForm/${contactFormData.data[0].form_aid}` // update
  ...
  const initVal = {
    isUpdateContactForm: itemEdit,
    form_img: contactFormData?.data?.[0]?.form_img ?? "",
    form_address: contactFormData?.data?.[0]?.form_address ?? "",
    form_contacts: contactFormData?.data?.[0]?.form_contacts ?? "",
    form_facebook_link: contactFormData?.data?.[0]?.form_facebook_link ?? "",
    form_linkedin_link: contactFormData?.data?.[0]?.form_linkedin_link ?? "",
    form_youtube_link: contactFormData?.data?.[0]?.form_youtube_link ?? "",
    form_instagram_link: contactFormData?.data?.[0]?.form_instagram_link ?? "",
    form_tiktok_link: contactFormData?.data?.[0]?.form_tiktok_link ?? "",

    form_img_old: contactFormData?.data?.[0]?.form_img ?? "",
    pendingDeleteFile: [],
  };
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                form_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
