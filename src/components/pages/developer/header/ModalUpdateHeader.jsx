...
const ModalUpdateHeader = ({ itemEdit, headerData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        headerData?.data?.length
          ? `${apiVersion}/header/${headerData.data[0].header_aid}` // update
          : `${apiVersion}/header`, // create
  ...
  const initVal = {
    ...
    header_logo_img_old: headerData?.data?.[0]?.header_logo_img ?? "",
    pendingDeleteFile: [],
  };
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                header_logo_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateHeader;
