...
const CategoryTable = ({ setItemEdit }) => {
  ...
  const {
    ...
    queryKey: ["packages-category", onSearch, store.isSearch],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/packages-category/search`, // search endpoint
        `${apiVersion}/packages-category/page/${pageParam}`, // list endpoint
  ...
  const handleDelete = (item) => {
    ...
    setIsData(item.packages_category_name);
    setIsId(item.packages_category_aid);
  };
    ...
    <>
                ...
                {page?.data.map((item, key) => {
                  return (
                    <tr key={key} className="place-content-start text-[14px]">
                      ...
                      <td>
                        {item.packages_category_is_active === 1 ? (
                          <Status text="Active" />
                        ) : (
                      ...
                      <td className="place-content-start">
                        {item.packages_category_url}
                      </td>
                      <td className="flex items-center gap-3 justify-end mt-2 lg:mt-0">
                        {item.packages_category_is_active ? (
                          <>
                            <button
      ...
      {store.isDelete && (
        <ModalDelete
          ...
          mysqlEndpoint={`${apiVersion}/packages-category/${id}`}
          item={isData}
        />
      )}
      {store.isArchive && (
        <ModalArchive
          ...
          queryKey={"packages-category"}
          mysqlEndpoint={`${apiVersion}/packages-category/active/${id}`}
          item={isData}
          archive={isArchiving}
        />
      )}
