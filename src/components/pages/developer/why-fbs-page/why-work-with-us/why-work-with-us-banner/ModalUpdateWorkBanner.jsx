...
const ModalUpdateWorkBanner = ({ itemEdit, workData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        workData?.data?.length
          ? `${apiVersion}/work/${workData.data[0].work_banner_aid}` // update
          : `${apiVersion}/work`, // create
        workData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateWork: itemEdit,
    work_banner_title: workData?.data?.[0]?.work_banner_title ?? "",
    work_banner_title_bold: workData?.data?.[0]?.work_banner_title_bold ?? "",
    work_banner_description: workData?.data?.[0]?.work_banner_description ?? "",
    work_banner_button_text: workData?.data?.[0]?.work_banner_button_text ?? "",
    work_banner_img: workData?.data?.[0]?.work_banner_img ?? "",

    work_banner_img_old: workData?.data?.[0]?.work_banner_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Why Work With Us Banner
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                work_banner_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateWorkBanner;
