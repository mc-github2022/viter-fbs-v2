...
const ModalUpdateWorkTeamsWeb = ({ itemEdit, workTeamsData }) => {
  ...
  const initVal = {
    ...
    work_teams_web_name: workTeamsData?.data?.[0]?.work_teams_web_name ?? "",
    work_teams_web_position:
      workTeamsData?.data?.[0]?.work_teams_web_position ?? "",
    work_teams_web_img: workTeamsData?.data?.[0]?.work_teams_web_img ?? "",

    work_teams_web_img_old: workTeamsData?.data?.[0]?.work_teams_web_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Web Supervisor
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                work_teams_web_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateWorkTeamsWeb;
