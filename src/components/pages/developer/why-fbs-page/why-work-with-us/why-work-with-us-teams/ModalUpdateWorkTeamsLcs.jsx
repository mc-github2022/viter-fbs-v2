...
const ModalUpdateWorkTeamsLcs = ({ itemEdit, workTeamsData }) => {
  ...
  const initVal = {
    ...
    work_teams_lcs_name: workTeamsData?.data?.[0]?.work_teams_lcs_name ?? "",
    work_teams_lcs_position:
      workTeamsData?.data?.[0]?.work_teams_lcs_position ?? "",
    work_teams_lcs_img: workTeamsData?.data?.[0]?.work_teams_lcs_img ?? "",

    work_teams_lcs_img_old: workTeamsData?.data?.[0]?.work_teams_lcs_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Web & LCS Supervisor
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                work_teams_lcs_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateWorkTeamsLcs;
