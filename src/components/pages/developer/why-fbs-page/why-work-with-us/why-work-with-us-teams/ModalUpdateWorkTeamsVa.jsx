...
const ModalUpdateWorkTeamsVa = ({ itemEdit, workTeamsData }) => {
  ...
  const initVal = {
    isUpdateWorkTeams: itemEdit,
    work_teams_va_name: workTeamsData?.data?.[0]?.work_teams_va_name ?? "",
    work_teams_va_position:
      workTeamsData?.data?.[0]?.work_teams_va_position ?? "",
    work_teams_va_img: workTeamsData?.data?.[0]?.work_teams_va_img ?? "",

    work_teams_va_img_old: workTeamsData?.data?.[0]?.work_teams_va_img ?? "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} VA Supervisor</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                work_teams_va_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateWorkTeamsVa;
