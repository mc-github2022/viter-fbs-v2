...
const ModalUpdateWorkTeamsOm = ({ itemEdit, workTeamsData }) => {
  ...
  const initVal = {
    ...
    work_teams_om_name: workTeamsData?.data?.[0]?.work_teams_om_name ?? "",
    work_teams_om_position:
      workTeamsData?.data?.[0]?.work_teams_om_position ?? "",
    work_teams_om_img: workTeamsData?.data?.[0]?.work_teams_om_img ?? "",

    work_teams_om_img_old: workTeamsData?.data?.[0]?.work_teams_om_img ?? "",
    pendingDeleteFile: [],
  };
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">
            {itemEdit ? "Edit" : "Add"} Operating Manager
          </h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values) => {
              ...
              const data = {
                ...values,
                work_teams_om_img: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
...
export default ModalUpdateWorkTeamsOm;
