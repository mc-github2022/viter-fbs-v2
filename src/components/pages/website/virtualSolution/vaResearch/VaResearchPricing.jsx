...
const VaResearchPricing = ({ pageName, businessTitlesData }) => {
  ...
  const selectedCategory =
    packagesListData?.data?.find(
      (item) => item.packages_category_url === "va-business-support"
    )?.packages_category_url || "va-business-support";
            ...
            {filteredItems.length > 3 ? (
              <Slider ref={sliderRef} {...eventsSliderSettings}>
                {filteredItems.map((price, key) => (
                  <div
                    key={key}
                    className={`priceItem mb-5 lg:mb-0 relative z-[1] text-light text-center p-10 rounded-lg  md:w-[400px] min-h-[400px] lg:min-h-[900px] ${
                ...
                <div className="md:grid md:place-items-center">
                  <div className="wrapper md:flex md:flex-row md:flex-wrap gap-6 items-center place-content-center">
                    {filteredItems.map((price, key) => (
                      <div
                        key={key}
                        className={`priceItem mb-5 lg:mb-0 relative z-[1] text-light text-center p-10 rounded-lg addShadow md:w-[400px] min-h-[400px] lg:min-h-[900px] ${
                          price.packages_list_is_highlighted === 1
                            ? "bg-primary"
                            : "bg-customGray !text-dark"
                        } grid `}
